...
##    raise NotImplementedError
    ...
    if board.is_win():
        # the evaluate function is called under either of the two situations:
        #(1). depths < =0   (2). game is over, one player has won 
        # the other player has just drop a token
        # the board is now belong to the current player
        # so the winner must be the other player
        return -1000 + board.num_tokens_on_board()

    score = board.longest_chain(current_player_id) * 10
    for i in range(board.board_width):
        for j in range(board.board_height):
            if board.get_cell(i, j) == current_player_id:
                score -= abs(i-3)
            elif board.get_cell(i, j) == other_player_id:
                score += abs(i-3)
    return score
...
##    raise NotImplementedError
    ...
    flag = 1  # flag = 1 indicate Max level, -1 Min level
    for row, new_board in board.get_next_moves_fn(board):
        val = alpha_beta(new_board, -1*flag, depth-1, eval_fn,
...
def alpha_beta(board, flag, depth, eval_fn, get_next_moves_fn,is_terminal_fn, alpha, beta):
    # 
...
##    raise NotImplementedError
    ...
    current_chain_set = board.chain_cells(current_player)
    other_chain_set = board.chain_cells(other_player)

    score = 0
    for item in current_chain_set:
        score += len(item)**2
    for item in other_chain_set:
        score -= len(item)**2
